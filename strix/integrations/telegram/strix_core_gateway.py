...
class StrixCoreGatewayResult:
    """Result returned by the optional real STRIX core gateway."""
...
class StrixCoreGateway:
    """Canonical optional bridge from Telegram lab messages into STRIX Core.
    ...
    async def handle_message(self, chat_id: str, user_id: str, text: str) -> StrixCoreGatewayResult:
        imports = self._load_imports()
        if imports is None:
            return StrixCoreGatewayResult(
                available=False,
                handled=False,
